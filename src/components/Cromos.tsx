...
const Cromos: React.FC = () => {
  ...
  const handleSaveInfo = async () => {
    ...
    if (error) {
      ...
      alert("No se pudieron guardar los cambios. Verifica las políticas RLS.");
    } else {
          ...
          <div style={detailCard} onClick={e => e.stopPropagation()}>
            ...
            <div style={detailInfoSection}>
                <div style={{marginBottom: '20px'}}>
                    ...
                    <div style={{fontSize: '11px', color: '#555', marginTop: '5px', letterSpacing: '1px'}}>SISTEMA ID: #{showFullImage.id}</div>
                </div>
...
const btnCloseAbsolute: React.CSSProperties = {
  position: 'absolute',
  top: '15px',
  right: '15px',
...
const detailImageSection: React.CSSProperties = {
  flex: '1.1',
  background: '#000',
  display: 'flex',
  alignItems: 'center',
...
const detailImg: React.CSSProperties = {
  maxWidth: '100%',
  maxHeight: '100%',
  objectFit: 'contain',
  borderRadius: '8px',
...
const detailInfoSection: React.CSSProperties = {
  flex: '1',
  padding: '35px',
  display: 'flex',
...
const techScrollArea: React.CSSProperties = {
  flex: 1,
  overflowY: 'auto',
  paddingRight: '10px',
  marginBottom: '20px',
...
const btnCloseDetail: React.CSSProperties = {
  width: '100%',
  padding: '14px',
  background: '#eab308',
  color: '#000',
